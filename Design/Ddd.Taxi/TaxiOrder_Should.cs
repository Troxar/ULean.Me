...
namespace Ddd.Taxi.Domain
{
    ...
    public class TaxiOrder_Should
    {
        ...
        [Test]
        public void TwoOrdersHasDifferentIds()
        {
            var taxiApi = CreateApi();
            var order1 = taxiApi.CreateOrderWithoutDestination("Sherlock", "Holmes", "Baker", "221b");
            var order2 = taxiApi.CreateOrderWithoutDestination("John", "Watson", "Baker", "221b");
            Assert.AreNotEqual(order1.Id, order2.Id);
        }

        [Test]
        public void ThrowInformativeExceptions_OnInvalidActions()
        {
            var taxiApi = CreateApi();
            time = new DateTime(2017, 1, 1);
            var order = taxiApi.CreateOrderWithoutDestination("John", "Doe", "Street1", "building1");
